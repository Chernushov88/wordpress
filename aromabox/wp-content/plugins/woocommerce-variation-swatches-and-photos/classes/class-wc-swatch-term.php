...
class WC_Swatch_Term {
	...
	public function get_output( $placeholder = true, $placeholder_src = 'default' ) {
		...
		if ( $this->type == 'photo' || $this->type == 'image' ) {
			$picker .= '<a href="' . $href . '"  title="' . esc_attr( $this->term_label ) . '" class="' . $anchor_class . '">';
		...
		$out = '<div class = "variate-card-wrapper"><div class = "name-img-wrapp"><div class="select-option swatch-wrapper' . ( $this->selected ? ' selected' : '' ) . '" data-attribute="' . esc_attr( $this->taxonomy_slug ) . '" data-value="' . esc_attr( $this->term_slug ) . '">';
    ...
    print_r($this);
		$out .= '</div>';
       $out .= '<div class="term_label-var">'. $this->term_label ;
        $out .= '</div></div>';
		
		       $out .= '<div class="variate-price-btn-wrap"><div class="term_label-price">999 руб' ;
        $out .= '</div>';
			       $out .= '<div class="term_label-button"><button type="submit" class="single_add_to_cart_button button alt">В корзину</button>' ;
        $out .= '</div></div></div>';	
