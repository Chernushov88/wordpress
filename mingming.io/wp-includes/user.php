	...
	if ( ! wp_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( '<strong>错误</strong>: 您输入的用户名%s的密码不正确。' ),
...
function wp_authenticate_email_password( $user, $email, $password ) {
	...
	if ( empty( $email ) || empty( $password ) ) {
		...
		if ( empty( $email ) ) {
			...
			$error->add( 'empty_username', __( '<strong>错误</strong>: 电子邮件字段为空。' ) );
		}
	...
	if ( ! wp_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( '<strong>错误</strong>: 您输入的电子邮件地址%s的密码不正确。' ),
				'<strong>' . $email . '</strong>'
			) .
			' <a href="' . wp_lostpassword_url() . '">' .
			__( '丢失了你的密码？' ) .
...
function wp_authenticate_spam_check( $user ) {
	if ( $user instanceof WP_User && is_multisite() ) {
		...
		if ( $spammed ) {
			return new WP_Error( 'spammer_account', __( '<strong>错误</strong>: Your account has been marked as a spammer.' ) );
		}
	}
	return $user;
}
...
function get_password_reset_key( $user ) {
	...
	if ( ! ( $user instanceof WP_User ) ) {
		return new WP_Error( 'invalidcombo', __( '<strong>错误</strong>: There is no account with that username or email address.' ) );
	}
...
function register_new_user( $user_login, $user_email ) {
	...
	if ( '' == $sanitized_user_login ) {
		$errors->add( 'empty_username', __( '<strong>错误</strong>: Please enter a username.' ) );
	} elseif ( ! validate_username( $user_login ) ) {
		$errors->add( 'invalid_username', __( '<strong>错误</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.' ) );
		$sanitized_user_login = '';
	} elseif ( username_exists( $sanitized_user_login ) ) {
		$errors->add( 'username_exists', __( '<strong>错误</strong>: This username is already registered. Please choose another one.' ) );

	} else {
		/** This filter is documented in wp-includes/user.php */
		$illegal_user_logins = (array) apply_filters( 'illegal_user_logins', array() );
		if ( in_array( strtolower( $sanitized_user_login ), array_map( 'strtolower', $illegal_user_logins ), true ) ) {
			$errors->add( 'invalid_username', __( '<strong>错误</strong>: Sorry, that username is not allowed.' ) );
		}
	}

	// Check the email address.
	if ( '' == $user_email ) {
		$errors->add( 'empty_email', __( '<strong>错误</strong>: Please type your email address.' ) );
	} elseif ( ! is_email( $user_email ) ) {
		$errors->add( 'invalid_email', __( '<strong>错误</strong>: The email address isn&#8217;t correct.' ) );
		$user_email = '';
	} elseif ( email_exists( $user_email ) ) {
		$errors->add( 'email_exists', __( '<strong>错误</strong>: This email is already registered, please choose another one.' ) );
	}
	...
	if ( ! $user_id || is_wp_error( $user_id ) ) {
		$errors->add(
			...
			sprintf(
				...
				__( '<strong>错误</strong>: Couldn&#8217;t register you&hellip; please contact the <a href="mailto:%s">webmaster</a> !' ),
...
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		if ( ! is_email( $_POST['email'] ) ) {
			$errors->add(
				'user_email',
				__( '<strong>错误</strong>: The email address isn&#8217;t correct.' ),
				array(
					'form-field' => 'email',
				)
			);
		...
		if ( email_exists( $_POST['email'] ) ) {
			$errors->add(
				'user_email',
				__( '<strong>错误</strong>: The email address is already used.' ),
				array(
					'form-field' => 'email',
				)
			);
