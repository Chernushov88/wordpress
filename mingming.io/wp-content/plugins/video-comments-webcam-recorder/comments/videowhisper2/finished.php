...
setcookie("recIdCookie", $wpdb->insert_id, time()+3600*24,'/');
	...
	if ($options['videosharevod'])
	{
		...
		$postType = $optionsVSV['custom_post'];
		if (!$postType) $postType = 'video';


		global $wpdb;
		$postID = $wpdb->get_var( $sql = "SELECT ID FROM $wpdb->posts WHERE post_name = '" . sanitize_file_name($streamname) . "' and post_type='$postType' LIMIT 0,1" );

		$playercode = "[videowhisper_player video=\"$postID\" fallback_enabled=\"0\"]";

		if (!$postID) $playercoder .= 'Error - ID not found. ( ' . $sql.' )';
...
//wp comment
		var commentElement = doc.getElementById("comment");

		if (!commentElement)
		{
			commentElements = doc.getElementsByName("comment");
			if (commentElements) commentElement = commentElements[0];
		}

//bp activity
		var commentElement = doc.getElementById("whats-new");

		if (!commentElement)
		{
			commentElements = doc.getElementsByName("whats-new");
			if (commentElements) commentElement = commentElements[0];
		}


		if (commentElement) commentElement.value = "<?php echo $playercode;?>";
		else window.alert('Error: Element (comment) or (whats-new) not found by id or name to insert video on originating page!');
	}
	else window.alert('Error: Window opener not found to insert comment video!');
