...
if (file_exists($file = $video_from . $vid  . ".flv"))
	if ($options['videosharevod'] && class_exists("VWvideoShare"))
	{
		global $current_user;
		get_currentuserinfo();

		VWvideoShare::importFile($file, sanitize_file_name($stream), $current_user->ID, sanitize_file_name($current_user->display_name), '', 'webcam', $description = $current_user->display_name . ' webcam recording');
...
?>firstParameter=fix&loadstatus=1