...
$showMessage = '';
if ($options['finishedMessage']) $showMessage = 'true';
...
if($embedmode == 0)
{
	...
	case 'videosharevod';
		...
		global $wpdb;
		$postID = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_name = '" . sanitize_file_name($streamname) . "' and post_type='$postType' LIMIT 0,1" );
...
echo stripslashes($options['finishedMessage']);
...
else if (!showMessage) document.location = '<?php echo site_url(); ?>';
</script>
