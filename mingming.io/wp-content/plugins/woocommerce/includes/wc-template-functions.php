...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			...
			$page_title = sprintf( __( '搜索结果: &ldquo;%s&rdquo;', 'woocommerce' ), get_search_query() );
...
if ( ! function_exists( 'woocommerce_template_loop_product_title' ) ) {
	...
	function woocommerce_template_loop_product_title() {
		echo '<h2 class="' . esc_attr( apply_filters( 'woocommerce_product_loop_title_classes', 'woocommerce-loop-product__title' ) ) . '">' . get_the_title() . '<div class="category">博客</div></h2>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				...
				'title'    => sprintf( __( '评论 (%d)', 'woocommerce' ), $product->get_review_count() ),
				'priority' => 30,
...
function woocommerce_output_all_notices() {
	echo '<div class="woocommerce-notices-wrapper test">';
	wc_print_notices();
	echo '</div>';
}
