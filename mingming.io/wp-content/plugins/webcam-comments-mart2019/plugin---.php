...
function action_copy_mp4file( $comment_ID, $comment_approved, $commentdata ) {
	...
	if( stristr($commentdata['comment_content'], '[videocomment]') )// See if exists [videocomment] in comment
	{
		...
		$newfile = ABSPATH.'wp-content/mp4/'.$date_Y_m.'/'.$date_day.'/'.$comment_ID.'.mp4';
...
function webcamcomments_comment_form()
{
	...
	echo 'Здесь Вы можете записать только видеокомментарий с вебкамеры. Текстовые комментарии не принимаются. У Вас есть 30 секунд. Нажмите "Start Recording", говорите, затем "Stop Recording",
	затем "Отправить комментарий". Если Вы на смартфоне, говорите прямо в микрофон. Если будет плохо слышно, комментарий будет удалён.<br>';//Post comment
