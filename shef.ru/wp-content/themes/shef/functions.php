...
function twentyseventeen_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => __( 'sidebar-phone', 'twentyseventeen' ),
			'id'            => 'sidebar-phone',
			'description'   => __( 'Add widgets here to appear in your footer.', 'twentyseventeen' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
if(!is_home()){ 
   echo '<a href="'.site_url().'">Главная</a> '.$sep ;
	if(is_single()){ // записи
	the_category(', ');
	echo $sep ;
	the_title();
	...
	elseif (is_page()) { // страницы
		if ($post->post_parent ) {
			...
			foreach ($breadcrumbs as $crumb) echo $crumb . $sep ;
		}
		echo the_title();
	}
	elseif (is_category()) { // категории
		...
		if ($current_cat->parent != 0) 
			echo(get_category_parents($parent_cat, TRUE, $sep));
		single_cat_title();
	}
   ...
   if($pageNum>1)
      echo '<a href="'.site_url().'">Главная</a> '.$sep.$pageNum.'-я страница';
   else
